...
Infrastructure lives in [infra](https://github.com/charemma/infra),
VPS config in [nixos-config](https://github.com/charemma/nixos-config).

Deployment is fully managed via GitOps using ArgoCD. The ArgoCD Application
is defined in [infra/vps/apps/charemma-web.yaml](https://github.com/charemma/infra/blob/main/vps/apps/charemma-web.yaml)
...
3. ArgoCD Image Updater detects the new image tag and updates `k8s/` via Kustomize
